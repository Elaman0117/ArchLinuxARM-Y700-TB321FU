...
# Build a FAT boot image for Lenovo Y700 (TB321FU) containing:
#   - BOOTAA64.EFI (UEFI bootloader)
#   - QCOMRAMP.EFI (Qualcomm direct-boot GRUB EFI)
#   - Linux kernel Image
#   - Device Tree Blob (DTB)
#   - GRUB configuration with Arch Linux ARM compatible boot parameters
#
# The boot image structure is device-specific, not distro-specific.
# The key Arch-specific difference is the init path:
#   Arch uses /sbin/init (systemd) and /usr/lib/systemd/systemd as fallback.
#   Ubuntu uses /sbin/init as well, but Arch's systemd paths may differ.
# We also add console=tty1 which is Arch's default.
...
Environment inputs:
  ...
  OUTPUT_PREFIX              default: archlinuxarm-y700
  ...
  BOOT_IMAGE_SIZE            default: 256M
  BOOT_FAT_BITS              12|16|32, default: 32
  BOOT_FAT_LABEL             default: ALARMGRUB
...
OUTPUT_PREFIX=${OUTPUT_PREFIX:-archlinuxarm-y700}
BOOT_TEMPLATE_IMAGE=${BOOT_TEMPLATE_IMAGE:-${BOOT_TEMPLATE_IMAGE_URL:-}}
BOOT_IMAGE_SIZE=${BOOT_IMAGE_SIZE:-256M}
BOOT_FAT_BITS=${BOOT_FAT_BITS:-32}
BOOT_FAT_LABEL=${BOOT_FAT_LABEL:-ALARMGRUB}
...
[ -n "${KERNEL_IMAGE:-}" ] && [ -f "$KERNEL_IMAGE" ] || die "KERNEL_IMAGE is required"
[ -n "${DTB_FILE:-}" ] && [ -f "$DTB_FILE" ] || die "DTB_FILE is required"
[ -n "$BOOT_TEMPLATE_IMAGE" ] || { [ -n "${BOOTAA64_EFI:-}" ] && [ -f "$BOOTAA64_EFI" ]; } || die "BOOTAA64_EFI or BOOTAA64_EFI_URL is required without BOOT_TEMPLATE_IMAGE"
...
if [ -n "${QCOMRAMP_EFI:-}" ]; then
  [ -f "$QCOMRAMP_EFI" ] || die "QCOMRAMP_EFI does not exist: $QCOMRAMP_EFI"
...
if [ -n "$BOOT_TEMPLATE_IMAGE" ]; then
  ...
  # Arch Linux ARM GRUB configuration
  # Key differences from Ubuntu:
  #   - init=/sbin/init (systemd) is the default; Arch doesn't use upstart
  #   - console=tty1 is Arch's default console
  #   - log_buf_len=64M and other debug params stay the same (kernel-level)
